...
function populateStates(countryElementId, stateElementId) {
	...
	for (var i = 0; i < state_arr.length; i++) {
		//Check if a default state is selected, adn select it
		if (b !== undefined) {
			if (state_arr[i] === b) {
				stateElement.options[stateElement.length] = new Option(state_arr[i], state_arr[i], true, true);
			} else {
...
function populateCountries(countryElementId, stateElementId) {
	...
	let a = countryElement.value;
	countryElement.length = 0;
	countryElement.options[0] = new Option('Select Country', '-1');
	countryElement.selectedIndex = 0;
	for (var i = 0; i < country_arr.length; i++) {
		//Check if a default country is selected, adn select it
		if (a !== undefined) {
			// console.log(countryElement.dataset.selected);
			if (country_arr[i] === a) {
